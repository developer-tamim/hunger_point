<template>
  <Layout>
    <div class="max-w-6xl p-6 mx-auto space-y-6">
      ...
      <div class="flex items-center justify-between p-6 bg-white shadow-sm rounded-xl">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-3xl font-bold text-orange-600">{{ recipeStore.recipes.length }}</p>
          </div>
        </div>
        <button 
          @click="openAddModal"
      ...
      <!-- Recipe Grid -->
      <div v-if="sortedRecipes.length > 0" class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div 
        ...
        >
          <div class="flex items-start justify-between">
            <h3 class="flex-1 pr-4 text-lg font-semibold">{{ recipe.title }}</h3>
          ...
          <div class="flex flex-wrap items-center gap-2 mt-3">
            ...
            <span 
              :class="[
                ...
                recipe.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
              ]"
            >
    ...
    <div v-if="showFormModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div class="w-full max-w-2xl max-h-screen overflow-y-auto bg-white shadow-xl rounded-xl">
        <div class="flex items-center justify-between p-6 border-b">
          <h3 class="text-xl font-semibold">{{ isEditMode ? 'Edit Recipe' : 'Add New Recipe' }}</h3>
          <button @click="closeFormModal" class="text-2xl leading-none text-gray-500 hover:text-gray-700">
            ×
          </button>
        </div>
        ...
        <div class="p-6">
          <form @submit.prevent="submitRecipe" class="space-y-4">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <input v-model="form.title" type="text" placeholder="Recipe Title *" required
            ...
            <div>
              <label class="block mb-1 text-sm font-medium">Ingredients *</label>
              <textarea v-model="form.ingredients" rows="4" placeholder="One ingredient per line..." required
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"></textarea>
            </div>

            <div>
              <label class="block mb-1 text-sm font-medium">Instructions *</label>
              <textarea v-model="form.steps" rows="6" placeholder="Step-by-step instructions..." required
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"></textarea>
            </div>
    ...
    <div v-if="showDetailsModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div class="w-full max-w-3xl max-h-screen overflow-y-auto bg-white shadow-xl rounded-xl">
        <div class="flex items-center justify-between p-6 border-b">
          <h3 class="text-xl font-semibold">{{ selectedRecipe.title }}</h3>
          <button @click="closeDetailsModal" class="text-2xl leading-none text-gray-500 hover:text-gray-700">
            ×
          </button>
        </div>
        ...
        <div class="p-6 space-y-4">
          <div class="flex items-center space-x-3">
            ...
            <span 
              :class="[
                'px-2 py-1 rounded-full text-xs font-medium',
                selectedRecipe.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
              ]"
            >
          ...
          <div>
            ...
            <p class="p-4 text-gray-600 whitespace-pre-line rounded-lg bg-gray-50">
...
onMounted(() => {
  if (recipeStore.recipes.length === 0) {
    recipeStore.addRecipe({
      ...
      category: 'Burger',
      ingredients: '• Ground beef 500g\n• Burger buns 4\n• Lettuce\n• Tomato\n• Onion\n• Cheese slices\n• Salt & Pepper',
      steps: '1. Form beef into 4 patties\n2. Season both sides\n3. Grill 4 mins each side\n4. Toast buns\n5. Assemble with toppings',
      isActive: true
    })
  }
})
